...
\item \texttt{pytest -q}: the full suite ($\sim$1 min). By file: \src{tests/test\_recovery.py} (known-truth recovery; stage-B reduction is the MLE), \src{tests/test\_metamorphic.py} (invariances), \src{tests/test\_failure\_modes.py} (every hazard raises or warns), \src{tests/test\_curvature\_diagnostic.py} (the $R^2$-evading hazard is gated), \src{tests/test\_crosscheck.py} (link robustness), \src{tests/test\_parsers.py} (free-text answer contract), \src{tests/test\_validate.py} (held-out fit machinery), \src{tests/test\_inference.py} (bootstrap smoke test, label-immunity and support-floor regressions), \src{tests/test\_input\_checks.py} (malformed input fails loudly, naming frame, column, and cause), \src{tests/test\_extrapolation.py} (weights beyond the anchor range are measured and warned about).
